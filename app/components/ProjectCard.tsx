...
import { ExternalLink, Github, Calendar, Sparkles, Play, X } from 'lucide-react';
            ...
            {project.videoUrl && (
              ...
              >
                <div className="flex h-12 w-12 items-center justify-center rounded-full bg-white/90 shadow-lg transition-transform hover:scale-110">
                  <Play className="h-5 w-5 ml-1 text-violet-600" />
                </div>
              </button>
            )}
          </div>
        ...
        <div className="p-6">
          ...
          <div className="flex gap-3">
            {project.videoUrl && (
              <button
                onClick={() => setShowVideo(true)}
                className="flex items-center gap-2 rounded-lg bg-gray-900 px-4 py-2 text-sm font-medium text-white transition-all hover:bg-gray-800 dark:bg-white dark:text-zinc-900 dark:hover:bg-zinc-200"
              >
